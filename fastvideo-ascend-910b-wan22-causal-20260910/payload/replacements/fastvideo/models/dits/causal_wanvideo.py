...
class CausalWanSelfAttention(nn.Module):
    ...
    def forward(self,
        ...
        if kv_cache is None and self.attn.backend == AttentionBackendEnum.TORCH_SDPA:
            # FlexAttention is CUDA/Inductor-specific.  The exact same
            # block-causal layout can be evaluated without materialising an
            # LxL mask by issuing one SDPA call per temporal block.  This is
            # intentionally the accuracy-first path used on Ascend NPU.
            x = self._forward_blockwise_sdpa(
                roped_query,
                roped_key,
                v,
            ...
            if block_mask is None:
                raise ValueError("block_mask is required for FlexAttention causal training")
            ...
            flex_impl = flex_attention if torch.cuda.is_available() else _flex_attention
            x = flex_impl(query=padded_roped_query.transpose(2, 1),
    ...
    def _forward_blockwise_sdpa(self,
                                q: torch.Tensor,
                                k: torch.Tensor,
                                v: torch.Tensor,
                                *,
        ...
        """Evaluate the causal training layout with exact dense SDPA.

        Splitting by temporal block avoids constructing the quadratic boolean
        mask used by FlexAttention.  Within the current block attention stays
        bidirectional, while every earlier block is visible as causal context.
        """
        frame_seqlen = int(frame_seqlen)
        block_frames = int(num_frame_per_block)
        if frame_seqlen <= 0 or block_frames <= 0:
            raise ValueError("frame_seqlen and num_frame_per_block must be positive")

        tokens_per_block = frame_seqlen * block_frames
        if teacher_forcing:
            if self.local_attn_size != -1:
                raise NotImplementedError("Windowed teacher-forcing SDPA is not implemented; "
                                          "use local_attn_size=-1")
            if q.shape[1] % 2 != 0:
                raise ValueError("Teacher-forcing sequence must contain equally sized clean and noisy halves")
            clean_tokens = q.shape[1] // 2
            if clean_tokens % frame_seqlen != 0:
                raise ValueError("Teacher-forcing token count is not divisible by frame_seqlen")

            clean_outputs: list[torch.Tensor] = []
            noisy_outputs: list[torch.Tensor] = []
            for block_start in range(0, clean_tokens, tokens_per_block):
                block_end = min(block_start + tokens_per_block, clean_tokens)
                clean_outputs.append(
                    self.attn(q[:, block_start:block_end], k[:, :block_end], v[:, :block_end]))

                noisy_q_start = clean_tokens + block_start
                noisy_q_end = clean_tokens + block_end
                context_k = torch.cat(
                    [k[:, :block_start], k[:, noisy_q_start:noisy_q_end]], dim=1)
                context_v = torch.cat(
                    [v[:, :block_start], v[:, noisy_q_start:noisy_q_end]], dim=1)
                noisy_outputs.append(self.attn(q[:, noisy_q_start:noisy_q_end], context_k, context_v))
            return torch.cat(clean_outputs + noisy_outputs, dim=1)

        if q.shape[1] % frame_seqlen != 0:
            raise ValueError("Causal token count is not divisible by frame_seqlen")
        outputs: list[torch.Tensor] = []
        for block_start in range(0, q.shape[1], tokens_per_block):
            block_end = min(block_start + tokens_per_block, q.shape[1])
            if self.local_attn_size == -1:
                context_start = 0
            else:
                context_start = max(0, block_end - self.local_attn_size * frame_seqlen)
            outputs.append(
                self.attn(q[:, block_start:block_end], k[:, context_start:block_end], v[:, context_start:block_end]))
...
class CausalWanTransformer3DModel(BaseDiT):
    ...
    def _forward_train(self,
        ...
        use_blockwise_sdpa = (len(self.blocks) > 0
                              and self.blocks[0].attn1.attn.backend == AttentionBackendEnum.TORCH_SDPA)
        if teacher_forcing and not use_blockwise_sdpa:
        ...
        elif not teacher_forcing and not use_blockwise_sdpa:
